...
const ModalCancelAppointment: React.FC<propsNextAppointments> = ({ nextAppointmentOfPatient, cancelAppointment, setShowModalToCancelAppointment }) => {
    ...
    return (
        ...
        <div className="fixed inset-[0px] w-screen h-screen z-[9999] flex items-center justify-center bg-[rgba(0,0,0,0.4)] backdrop-blur-[12px]">
            ...
            <div className="flex flex-col gap-[20px] max-[530px]:w-[250px] max-[600px]:p-[20px] justify-start items-start bg-[#fff] w-[90%] max-w-[400px] rounded-[14px] h-auto p-[55px] shadow-2xl relative">
                {/* Botón X para cerrar */}
                <div
                ...
                <>
                    <h2 className="text-2xl font-bold text-[#0047ba] mb-[20px] text-[1.8rem] max-[530px]:text-[1.5rem]">¿Desea cancelar el siguiente turno?</h2>

                    <div className="flex flex-col items-start justify-start gap-[10px]">
                        <p className="text-[1.2rem] text-[#1e335f] max-[530px]:text-[1rem]"><strong>Especialidad:</strong> {nextAppointmentOfPatient?.professional?.specialty?.name}</p>
                        <p className="text-[1.2rem] text-[#1e335f] max-[530px]:text-[1rem]"><strong>Profesional:</strong> {nextAppointmentOfPatient?.professional?.name} {nextAppointmentOfPatient?.professional?.lastname}</p>
                        <p className="text-[1.2rem] text-[#1e335f] max-[530px]:text-[1rem]"><strong>Fecha:</strong> {formatFecha(nextAppointmentOfPatient?.date)}</p>
                        <p className="text-[1.2rem] text-[#1e335f] max-[530px]:text-[1rem]"><strong>Hora:</strong> {formatHora(nextAppointmentOfPatient?.date)}</p>
                    </div>
                    ...
                    <div className="flex self-center gap-[10px] mt-8">
                        <button
                            ...
                            className="flex-1 w-[350px] max-[530px]:w-[200px] h-[40px] px-[30px] py-[5px] rounded-[12px] cursor-pointer border-none bg-[#ff4d4d] text-[#fff] hover:opacity-90 transition-all">
                            Cancelar Turno
                        </button>
                    </div>
                </>

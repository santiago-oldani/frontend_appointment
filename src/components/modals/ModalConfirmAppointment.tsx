...
const ModalConfirmAppointment: React.FC<ModalConfirmProps> = ({ setModalConfirm, confirmAppointment, assignedAppointment }) => {
    const [appointmentConfirmed, setAppointmentConfirmed] = useState<boolean>(false);
    const [secondsToRedirect, setSecondsToRedirect] = useState<number>(5000);
    ...
    return (
        ...
        <div className="fixed inset-[0px] w-screen h-screen z-[9999] flex items-center justify-center bg-[rgba(0,0,0,0.4)] backdrop-blur-[12px]">
            ...
            <div className="flex flex-col gap-[20px] justify-start items-start bg-[#fff] w-[90%] max-w-[400px] rounded-[14px] h-auto p-[30px] shadow-2xl relative">
                ...
                {appointmentConfirmed ? <ModalAppointmentChecked /> :
                    ...
                    <>
                        ...
                        <div className="flex flex-col items-start justify-start gap-[10px]">
                            <p className="text-[1.2rem] text-[#1e335f]"><strong>Especialidad:</strong> {assignedAppointment?.specialty?.name}</p>
                            <p className="text-[1.2rem] text-[#1e335f]"><strong>Profesional:</strong> {assignedAppointment?.professional?.name} {assignedAppointment?.professional?.lastname}</p>
                            <p className="text-[1.2rem] text-[#1e335f]"><strong>Fecha:</strong> {formatFecha(assignedAppointment?.date)}</p>
                            <p className="text-[1.2rem] text-[#1e335f]"><strong>Hora:</strong> {formatHora(assignedAppointment?.date)}</p>
                        </div>
                        ...
                        <div className="flex self-center gap-[10px] mt-8">
                            <button
                                onClick={() => setModalConfirm(false)}
                                className="flex-1 px-[30px] py-[15px] rounded-[12px] cursor-pointer border-none hover:bg-gray-50 transition-all"
                            >
                                Cancelar
                            </button>
                            <button

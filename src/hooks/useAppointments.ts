...
export const useAppointments = () => {
    ...
    async function showSpecialties(): Promise<boolean> {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/specialties`);
    ...
    async function selectSpecialty(specialtyId: number): Promise<void> {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/professionals/by-specialtyId?specialtyId=${specialtyId}`);
    ...
    async function getAppointmentsAvailable(date: Dayjs): Promise<void> {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/appointments/available?professionalId=${assignedAppointment?.professional?.id}&date=${formattedDate}`);
    ...
    async function confirmAppointment(appointmentId: number): Promise<boolean> {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/appointments/${appointmentId}/assign`, {
                method: "PUT",
    ...
    const getNextAppointments = async (patientId: number) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/appointments/reserved/${patientId}`);
            if (response.ok) {
                const data: Appointment[] = await response.json();
    ...
    function cancelAppointment(appointmentId: number, patientId: number): void {
        fetch(`${import.meta.env.VITE_API_URL}/api/appointments/cancel/${appointmentId}`, {
            method: "PATCH"
        })
            .then((res) => {
